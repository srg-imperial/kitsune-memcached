...
static item ** E_OPAQUE todelete = 0; /**DSU xfgen */
static int delcurr;
static int deltotal;
static conn *listen_conn;
static struct event_base * E_OPAQUE main_base; /**DSU xfgen */

/** EKIDEN: heaplist to track connections passed to libevent **/
kitsune_heaplist E_G(conn) kitsune_track_conns; /**DSU data */
...
static void conn_init(void) {
    /* Ekiden change */
    kitsune_heaplist_init(&kitsune_track_conns); /**DSU data */
...
int main (int argc, char **argv) E_NOTELOCALS /**DSU data */
{
    int c;
    struct in_addr addr;
    ...
    char *username = NULL;
    char *pid_file = NULL;
    struct passwd *pw = NULL; /* kitsune - added initializer */ /**DSU other */
    struct sigaction sa;
    struct rlimit rlim;

    kitsune_do_automigrate(); /**DSU data */

    MIGRATE_LOCAL(lock_memory); /**DSU data */
    MIGRATE_LOCAL(daemonize); /**DSU data */
    MIGRATE_LOCAL(maxcore); /**DSU data */
    MIGRATE_LOCAL(username); /**DSU data */
    MIGRATE_LOCAL(pid_file); /**DSU data */
    MIGRATE_LOCAL(pw); /**DSU data */
    MIGRATE_LOCAL(rlim); /**DSU data */
    ...
    { 
        ...
        thread_migrate(settings.num_threads);
#endif

        event_del(&updateevent); /**DSU other 1 */
    }
