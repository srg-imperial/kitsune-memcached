 ...
 */
typedef struct _LIBEVENT_THREAD { /**DSU xfgen_ignore */
    pthread_t thread_id;        /* unique ID of this thread */
    struct event_base * E_OPAQUE base;    /* libevent handle this thread uses */ /**DSU xfgen */
    struct event notify_event;  /* listen event for notify pipe */
    int notify_receive_fd;      /* receiving end of notify pipe */
    int notify_send_fd;         /* sending end of notify pipe */

    struct event dsu_event;  /* listen event for DSU pipe */ /**DSU other */
    int dsu_receive_fd;      /* receiving end of DSU pipe */ /**DSU other */
    int dsu_send_fd;         /* sending end of DSU pipe */ /**DSU other */

    CQ  new_conn_queue;         /* queue of new connections to handle */
} LIBEVENT_THREAD;

static LIBEVENT_THREAD * E_PTRARRAY(4) threads; /**DSU xfgen */
...
static void create_worker(void *(*func)(void *), void *arg) {
    ...
    if (ret = kitsune_pthread_create(&thread, &attr, func, arg)) { /**DSU threads */
...
/* kitsune */
static void thread_dsu_process(int fd, short _which, void *arg) { /**DSU other 7 */
...
static void setup_thread(LIBEVENT_THREAD *me) {
    ...
    /**DSU other 5 */
    /* Listen for DSU notification from the main thread. */
    event_set(&me->dsu_event, me->dsu_receive_fd,EV_READ | EV_PERSIST, thread_dsu_process, me);
    event_base_set(me->base, &me->dsu_event);

    if (event_add(&me->dsu_event, 0) == -1) {
        fprintf(stderr, "Can't monitor libevent DSU pipe\n");
        exit(1);
    }
...
void notify_threads_update() { /**DSU other 4 */
    for(int i=1; i<init_count; i++) {
        if (write(threads[i].dsu_send_fd, "", 1) != 1) {
            perror("Writing to thread notify pipe");
        }
    }
}
...
/**DSU other 12 */
#include <assert.h>
void thread_migrate(int num_threads) {
    int i;

    for(i=0; i<num_threads; i++) {
        if (event_del(&threads[i].dsu_event) == -1) assert(0);
        if (event_del(&threads[i].notify_event) == -1) assert(0);

        event_set(&threads[i].dsu_event, threads[i].dsu_receive_fd, EV_READ | EV_PERSIST, thread_dsu_process, &threads[i]);
        event_base_set(threads[i].base, &threads[i].dsu_event);
        event_add(&threads[i].dsu_event, 0);

        event_set(&threads[i].notify_event, threads[i].notify_receive_fd, EV_READ | EV_PERSIST, thread_libevent_process, &threads[i]);
        event_base_set(threads[i].base, &threads[i].notify_event);
        event_add(&threads[i].notify_event, 0);
    }
}
